...
| cairo.o3.column.memory.size                    | 16M               | Memory page size per column for out-of-order (O3) operations. Note that O3 operations use 2x RAM per column.                                                                                                                    |
...
| cairo.writer.misc.append.page.size             | 4k                | mmap page size for mapping small files. The default value is the OS page size (4k for Linux, 64K for Windows, and 16k for OSX [Apple M1 chip]). Overriding this rounds to the nearest (greater) multiple of the OS page size.                                 |
...
| line.udp.own.thread.affinity | -1           | -1 does not set thread affinity. The OS will schedule thread and it will be liable to run on random cores and jump between 0 or higher pins thread to give core. This property is only valid when the UDP receiver uses own thread. |
| line.udp.unicast             | false        | When true, UDP will unicast. Otherwise, it will multicast.                                                                                                                                                                             |
